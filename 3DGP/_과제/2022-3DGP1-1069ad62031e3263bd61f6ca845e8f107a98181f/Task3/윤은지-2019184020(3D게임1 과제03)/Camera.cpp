...
void CSpaceShipCamera::Rotate(float x, float y, float z)
{
	if (m_pPlayer && (x != 0.0f))
	{
		...
		//22.06.18
		//뾰족한 걸 둥글게
		//XMFLOAT3 temporaryY = XMFLOAT3(m_pPlayer->GetPosition().x, m_pPlayer->GetPosition().y/10, m_pPlayer->GetPosition().z);
		//
...
CFirstPersonCamera::CFirstPersonCamera(CCamera* pCamera) : CCamera(pCamera)
{
	...
	if (pCamera)
	{
		...
		한다. 이것은 스페이스-쉽 카메라의 로컬 y-축 벡터가 어떤 방향이든지 1인칭 카메라(대부분 사람인 경우)의 로컬 y축 벡터가 월드좌표의 y-축이 되도록
