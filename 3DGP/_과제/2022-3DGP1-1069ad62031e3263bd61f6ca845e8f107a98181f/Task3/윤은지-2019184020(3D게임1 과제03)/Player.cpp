...
//22.06.18
	//적군 위치 갱신
void CPlayer::Update(XMFLOAT3 xmf3Shift, float fTimeElapsed)
{
...
void CPlayer::Render(int badGuy, ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	CPlayer(nMeshes)
{
	...
	////22.06.18
	////적군 쉐이더
	///*CObjectsShader* pShader2 = new CObjectsShader();
...
void CAirplanePlayer::OnPrepareRender()
{
	...
	//XMMATRIX mtxRotate = XMMatrixRotationRollPitchYaw(XMConvertToRadians(0.0f), 90.0f, 0.0f);
	...
	nMeshes) : CPlayer(nMeshes)
{
	...
	//플레이어 위치 벡터의 y좌표가 지형의 높이보다 크고 중력이 작용하도록 플레이어를 설정하였으므로 플레이어는 점차적으로 하강하게 된다.*/ 
	...
	//CAirplaneMeshDiffused* pCubeMesh = new CAirplaneMeshDiffused(pd3dDevice, pd3dCommandList, 10.0f, 10.0f, 2.0f);
	XMFLOAT4 tmp;
	//22.06.19
//플레이어 색상
	if (1 == planeColor)
...
CTerrainPlayer::CTerrainPlayer(int who, ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList
	* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext, int
	nMeshes) : CPlayer(nMeshes)
{
...
//플레이어 색상
	...
	pCubeMesh = new CAirplaneMeshDiffused(tmp, pd3dDevice, pd3dCommandList, 10.0f, 10.0f, 2.0f);
	//if (1 == planeColor)
	//	pCubeMesh->CAirplaneMeshDiffused(XMFLOAT4(1, 1, 1, 1), pd3dDevice, pd3dCommandList, 10.0f, 10.0f, 2.0f);
	//pCubeMesh->ReleaseUploadBuffers();
...
중심이 직육면체의 가운데이므로 y 값에 메쉬의 높이의 절반을 더하면 플레이어의 위치가 된다.*/
	...
	float fHeight = pTerrain->GetHeight(xmf3PlayerPosition.x, xmf3PlayerPosition.z) + 6.0f;// 뾰족한 걸 둥글게
