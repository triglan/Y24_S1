...
CGameFramework::CGameFramework()
{
	...
	//
	m_pCamera->m_d3dViewport = { 0, 0, FRAME_BUFFER_WIDTH, FRAME_BUFFER_HEIGHT, 0.0f, 1.0f };
	m_pCamera->m_d3dScissorRect = { 0, 0, FRAME_BUFFER_WIDTH, FRAME_BUFFER_HEIGHT };
...
void CGameFramework::ProcessInput()
{
	...
	if (::GetKeyboardState(pKeyBuffer))
	{
		...
		//if (pKeyBuffer[VK_F4] & 0xF0) dwDirection |= DIR_toTheBattlefield;//전장으로
	}
	...
	//플레이어를 적군 진영으로 이동
	XMFLOAT3 xmf3Shift
		= XMFLOAT3(m_pPlayer->GetPosition().x - badGuy->GetPosition().x, m_pPlayer->GetPosition().y - badGuy->GetPosition().y, m_pPlayer->GetPosition().z - badGuy->GetPosition().z);
	...
	xmf3Shift
		= XMFLOAT3(m_pPlayer2->GetPosition().x - badGuy2->GetPosition().x, m_pPlayer2->GetPosition().y - badGuy2->GetPosition().y, m_pPlayer2->GetPosition().z - badGuy2->GetPosition().z);
