...
CAirplanePlayer::CAirplanePlayer()
{
	...
	//+ ½¯µå »ý¼º ¸Þ½¬, À§Ä¡
	m_Shields = new CGameObject();
	m_Shields->SetMesh(new CCubeMesh(6.0f, 6.0f, 6.0f)); 
	m_Shields->SetPosition(0.0f, 0.0f, 0.0f); 
	m_Shields->SetColor(RGB(0,255,180));
}
...
void CAirplanePlayer::Animate(float fElapsedTime)
{
	...
	if (m_ShieldCool > 0) {//½¯µå ÄðÅ¸ÀÓÀÏ¶§
		m_ShieldCool -= fElapsedTime;
	}
	else if (isShield) {//+ ½¯µå ¾÷µ¥ÀÌÆ®, ½¯µå ÄðÅ¸ÀÓÀÌ ´Ù µÇ¾úÀ» ¶§
		XMFLOAT3 playerPosition = GetPosition();
		m_Shields->SetPosition(playerPosition);
		m_Shields->Animate(fElapsedTime);//+½¯µå ²¨Áö´Â ¹®Á¦ ÇØ°á

		m_ShieldDur += fElapsedTime;//+ ½¯µå ÄðÅ¸ÀÓ °è»ê
		if (m_ShieldDur > 5.f) {
			isShield = false;
			m_ShieldCool = 5.f;
			m_ShieldDur = 0.f;
...
void CAirplanePlayer::Render(HDC hDCFrameBuffer, CCamera* pCamera)
{
	...
	CGameObject* pShieldObject = NULL;//+ ½¯µå ¿ÀºêÁ§Æ® ÂüÁ¶
	pShieldObject = m_Shields;

	CPlayer::Render(hDCFrameBuffer, pCamera);

	if (isShield) {//+ ½¯µå¸¦ Ä×À¸¸é ½¯µå¸¦ ÄÑ¶ó
		//XMFLOAT3 playerPosition = GetPosition();
		//m_Shields->SetPosition(playerPosition);
		m_Shields->Render(hDCFrameBuffer, pCamera);
	}
