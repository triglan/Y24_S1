...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case VK_CONTROL:
			...
			break;
		case 'Z':
			if (((CAirplanePlayer*)m_pPlayer)->m_ShieldCool <= 0 
				&& ((CAirplanePlayer*)m_pPlayer)->m_ShieldDur <= 0.1f ) {//+쿨타임 아닐 때, 쉴드 지속중 아닐때
				((CAirplanePlayer*)m_pPlayer)->isShield = true;
				((CAirplanePlayer*)m_pPlayer)->m_ShieldDur = 0;
			}
		default:
...
void CGameFramework::ProcessInput()
{
	...
	if (GetKeyboardState(pKeyBuffer))
	{
		...
		if (dwDirection) m_pPlayer->Move(dwDirection, 0.35f);//+플레이어 이동속도
	}
