...
CGameObject::CGameObject(float x, float y, float z)
{
	...
	EnemyMissileDelay = DefaultEnemyMissileDelay;
...
void CGameObject::AnimateMissile(float fTimeElapsed) {
	this->Move(MovingDirection, 100 * fTimeElapsed);
...
{
	if(isAlive)
		if (EnemyMissileDelay > 0)
			EnemyMissileDelay -= fTimeElapsed * 25;
}

void CGameObject::AnimateEnemyMissile(float fTimeElapsed) {
	Move(MovingDirection, 100 * fTimeElapsed);
