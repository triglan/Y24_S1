...
CGameFramework::CGameFramework()
{
	...
	_tcscpy_s(m_pszFrameRate, _T("3DGPGame_2 ("));
}
	...
	case WM_KEYDOWN:
		...
		case 'Z':  // 쉴드
			if (!m_pPlayer->isShield)
				m_pPlayer->isShield = true;
			else
				m_pPlayer->isShield = false;
			break;
		case 'E':
			m_pScene->isMainGame = true;
			break;

		case VK_SPACE:	//미사일 발사
			if(m_pScene->isMainGame)
				m_pScene->CreateMissile();
			else {
				m_pScene->isMainGame = true;
				m_pScene->elapsedTime = 0;
			}
			break;
		}
		break;
