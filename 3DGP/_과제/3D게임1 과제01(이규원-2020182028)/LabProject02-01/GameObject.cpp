...
CExplosiveObject::~CExplosiveObject()
{
	for (int i = 0; i < ENEMYBULLETS; i++) if (m_ppBullets[i]) delete m_ppBullets[i];
}
...
void CExplosiveObject::Render(HDC hDCFrameBuffer, CCamera* pCamera)
{
	...
	for (int i = 0; i < ENEMYBULLETS; i++) if (m_ppBullets[i]->m_bActive) m_ppBullets[i]->Render(hDCFrameBuffer, pCamera);//+ÃÑ Ãâ·Â
}
